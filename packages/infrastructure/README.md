...
metadata cannot prove a write-only value. Each stable root script first rebuilds
this package and stops before provider access when that build fails; Alchemy's
child process therefore cannot resolve a stale ignored `dist/**` adapter. After
apply, require exact provider
