...
- A SPEC that renames an exported tagged error must state the compatibility
  decision for persisted values, public payloads, independently deployed
  consumers, and external decoders. It must enumerate the declaration,
  self-type, literal tag, constructors, unions, catchers, guards, mappings,
  tests, and documentation affected, and require exact target encode/decode,
  old-tag rejection, and stable public-boundary behavior.
