...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- |
| Native ownership and source separation       | One shared stable stack produces a decoded desired plan and decoded native `Sync.plan`; every finding records its exact source                                          | A parallel drift engine, direct-provider mapper, ordinary plan presented as provider drift                | `report-drift.ts`, `alchemy.stable.run.ts`, focused typecheck and drift fixtures     | Reopened and corrected after live adapter failure; exact-source rerun pending |
...
| Desired-state classification                 | Create/update/retained-delete plan actions remain `desiredStatePlanChange`; destructive replacement/delete stays `destructiveDrift`; neither becomes live sync evidence | Successful plan, persisted state or desired change called provider drift                                  | Dedicated source-separation and destructive desired-change fixtures                  | Passed locally; live stale-baseline negative exposed nine changes             |
...
### Live drift qualification correction

The first provider-bound qualification reopened the implementation owner rather
than accepting a generic inconclusive receipt. Native CLI isolation proved
Preview state/provider readback itself was healthy and returned 155 unchanged
resources. The wrapper failure was local: it attempted to decode every native
provider attribute as the Vercel secret-ownership projection, then failed again
when a desired-plan row did not exist in the supplied manifest. The corrected
adapter decodes that projection only for
`vercelEnvironmentVariable`, derives manifest-absent plan resource kinds from
the decoded native provider type, and emits only a bounded phase enum when
native execution cannot be decoded. The stable stack now also provides every
manifest `removalPolicy` through Alchemy's `RemovalPolicy` Service. A focused
Alchemy lifecycle test proves an adopted resource removed from the next
manifest settles as `retained` with no provider write.

The next live result rejected a second plausible false green: ignored
`alchemy-stable-preview-current.json` was an older 150-resource baseline whose
four managed revisions belonged to `0689b63…`. Against persisted state it
correctly produced four environment updates and five retained
deployment-observation removals. The actual receipt-bearing post-apply owner,
`alchemy-stable-preview-99cf80b-after-apply.json` with digest `a8fcfd1e…`,
produced 155 desired no-ops and 155 native unchanged observations twice. Both
normalized classified reports matched exactly: 111 accepted metadata rows,
zero blocking rows and 44 `unknownSecretRevision` rows for write-only Vercel
environment values. The report remains honestly inconclusive because neither
the no-op desired plan nor unchanged metadata proves remote secret contents.

These exploratory runs do not accept the task: the corrected source bytes were
still uncommitted while the receipt candidate field named `b4bc6e1…`. The
correction must pass the complete repository gate and be committed; two fresh
runs must then bind that exact commit and post-apply manifest. Workflow source
