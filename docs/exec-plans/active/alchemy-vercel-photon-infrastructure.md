...
| Lint, config, commands and CI    | Change required | Add one root candidate-inventory command and four typed Config inputs; workflows and CI authority remain unchanged.                                                                              |
| Schemas, services and Layers     | Change required | Add the binding sink plus one fingerprint-only candidate inventory operation, safe errors/results and explicit live/memory Layers.                                                               |
| Tests and fixtures               | Change required | Add seven binding codec/lifecycle tests plus three candidate double-read, malformed-input/leak and cross-selection tests using production contracts.                                             |
| SPEC, tasks and plan             | Change required | Keep the task in progress; record completed custody, controlled-identity decision, isolated project/user topology, current webhook boundary and terminal-only audit.                             |
| Receipts and evidence            | Change required | Route the dated sanitized Preview isolation receipt and retain only exact provider/resource fingerprints, manifests, limitations, non-claims and rollback identity.                              |
| Rollout, rollback and archive    | Change required | Retain the rollout-created Preview project/user while later webhook/deployment proof remains open; rollback resolves only the exact Preview fingerprints and preserves the source project/users. |

Next, replace the ad-hoc inventory path with a durable fail-closed redacted
command, then implement the Photon Preview webhook lifecycle around the
existing binding sink. The selected user is adopted; no later slice may delete
it, add a second user, or weaken the source-binding readback. The terminal
