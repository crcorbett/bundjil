...
readback are current; no candidate has been selected yet.
...
| Isolation gate                                    | The accepted decision permits an existing controlled identity only after fingerprint-only inventory proves current bindings and Photon capability proves non-disruptive multi-project reuse. The service remains dormant during inventory.                                                                                                                                           | Provider credentials, historical shared users, an earlier shared-project Preview receipt, or successful user creation after an implicit detach cannot prove isolation.     | Sanitized candidate/provider inventory, exact before/after binding readback and this canonical decision owner. | In progress; no identity selected or user mutation yet.            |
...
Next, inventory controlled candidates and prove Photon's reuse/exclusivity
semantics before implementing the Photon Preview lifecycle around this sink.
If every existing identity is exclusive, the task must close with the SPEC's
truthful blocked fallback and zero shared-project mutation. The terminal
