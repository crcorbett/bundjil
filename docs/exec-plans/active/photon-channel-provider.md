...
| Reconcile Photon and prove Preview | In progress | The 15-second immutable candidate completed hosted inbound, Eve, accepted typing start, scoped SDK release, outbound acceptance, and handset delivery. Photon exposes no replay operation, so the remaining gate is the SPEC-owned Preview-only signed retry receipt: complete one fresh claim, return one intentional `503`, then require Photon's same-ID retry to produce `204`, zero second dispatch, and zero second reply before Production staging. |
