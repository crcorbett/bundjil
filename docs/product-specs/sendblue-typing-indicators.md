...
shell history. Record only event kinds, counts, statuses, safe typing
...
| Provider tests and build env                     | Change required | In the same provider task, update `apps/agent/test/{sendblue-config.test.ts,sendblue-client.test.ts}` and `turbo.json` `@bundjil/agent#build.env`. This precedes every feature build.                                                                                                                                               | Default/bounds/request/error/`TestClock` tests pass; `rg -n 'BUNDJIL_SENDBLUE_TYPING_MAX_DURATION_MILLIS' turbo.json apps/agent`; filtered agent build passes.                                                                                                                          |
...
| Preview operations                               | Change required | After local gates, follow `apps/agent/README.md` hosted runbook with the root `vercel deploy` command, fresh project token, and `vercel curl`. No provider webhook. Task: `prove-sendblue-typing-preview`.                                                                                                                          | Clean immutable Preview, protected route proof, exact start/stop/final counts and observations, zero Production inbound and zero retained fixture material.                                                                                                                             |
...
| Vercel environment mutation                      | N/A             | The accepted `120000` default means no new Vercel variable is required. The optional binding is documented and Turbo-scoped only.                                                                                                                                                                                                   | Hosted Config reports the default behavior; provider variable inventory has no new required key.                                                                                                                                                                                        |
