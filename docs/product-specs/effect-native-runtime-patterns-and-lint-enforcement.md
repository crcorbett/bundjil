...
| Boundary/effect checks           | Change required; delivered          | `tooling/boundary-audit.ts` rejects exported required, optional, or renamed `Schema.Defect` fields through `public-raw-cause` and arbitrary unknown fields retained by script-local tagged errors through `operator-raw-cause`; the exact exception registry remains unchanged. |
...
| Tests/fixtures                   | Change required; delivered          | Encoded error fixtures prove the closed shape; three public-error fixtures and two operator-error cases reject raw causes while accepting bounded diagnostics and immediate private adapter translation; deterministic time and installed lint fixtures remain unchanged.       |
