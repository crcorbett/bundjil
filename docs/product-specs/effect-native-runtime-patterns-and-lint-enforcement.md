...
| Schemas/services/Layers          | Targeted Change required; delivered | Exported errors are Schema-backed and cause-free; reusable R2 state composition retains typed Config failure; exact Alchemy roots alone satisfy the framework's infallible state-Layer type; command runtimes place acquisition inside their final error boundary; no service/helper wrapper was added. |
