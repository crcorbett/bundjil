...
| primitive failure | A private operator command has a bounded process-exit classification.                                                        | Migrate that command to an owner-local tagged error before extending rule scope; preserve receipt and exit semantics.                              | No generic shared command error or scripts-wide suppression.            |
...
| `lint/fixtures/effect-native.config.json`                                                    | Create, then update | Load the repository plugin and enable the original four rules, then the admitted primitive-failure correction, against the fixture files.                                                   |
...
| Lint plugin/config/tests         | Change required; delivered          | The existing plugin owns the original four Effect rules plus the primitive-failure correction, direct and installed fixtures, exact count-checked exceptions and approved zero-debt scopes; no second runner, plugin package, autofix or broad ignore was added.                                                    |
...
| Schemas/services/Layers          | Targeted Change required; delivered | Exported errors are Schema-backed and cause-free; migration counts and drift execution identity are branded; shared diagnostics are bounded; Codex and migration composition retain decoded redacted values; Production rollback eligibility is one cohesive Ref; service/Layer identities remain unchanged.        |
| Tests/fixtures                   | Change required; delivered          | Encoded error fixtures prove closed shapes; boundary fixtures cover Promise coordination, redacted decoder round-trips, ambient random identities, causes, exported Data errors, aliased fields, drift run binding, and manifest digest retention; seeded Random and Production transition fixtures remain focused. |
