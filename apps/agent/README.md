...
Schemas. It reuses `@bundjil/eve` contracts and `@bundjil/store` services
instead of duplicating their fields. The Codex proxy bearer Schema remains
app-owned because Eve evaluates it inside the agent bundle and a package-owned
Redacted Schema crosses Effect registry instances during hosted builds. Decode
complete webhook, config, and completed-event projections at their boundaries,
encode outbound provider values, and use `Match` for material decoded unions.
Tagged errors and the read-only deployment preflight retain checked
diagnostic/transport strings.
